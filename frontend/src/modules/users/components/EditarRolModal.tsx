...
import { Button, ValidationMessage } from '../../../components/shared';
...
// ============================================================================
// STYLED COMPONENTS (migrados a theme)
// ============================================================================
...
const Input = styled.input<{ $hasError?: boolean; disabled?: boolean }>`
  ...
  font-size: ${TYPOGRAPHY.fontSize.body};
  transition: all ${TRANSITIONS.normal};
  background: ${props => props.disabled ? COLORS.background : COLORS.white};
  cursor: ${props => props.disabled ? 'not-allowed' : 'text'};

  &:focus {
    outline: none;
    border-color: ${props => props.$hasError ? COLORS.danger : COLORS.primary};
    box-shadow: ${props => props.disabled ? 'none' : `0 0 0 3px ${props.$hasError ? COLORS.dangerBg : COLORS.primaryLight}`};
  }
`;
...
const EditarRolModal: React.FC<EditarRolModalProps> = ({ role, onClose, onSubmit }) => {
  ...
  const loadValidPermissions = async () => {
    try {
      ...
      console.warn('[EditarRolModal] /roles/permissions not available, using local metadata');
    }
