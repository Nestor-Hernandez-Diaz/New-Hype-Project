...
print("0. Running SQL migrations...")

sql_commands = [
    "ALTER TABLE tickets_soporte ADD COLUMN usuario_id BIGINT NULL AFTER tenant_id;",
    """CREATE TABLE IF NOT EXISTS respuestas_ticket (
        ...
        CONSTRAINT fk_respuestas_ticket FOREIGN KEY (ticket_id)
            REFERENCES tickets_soporte(id) ON DELETE CASCADE
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;"""
]

for i, sql in enumerate(sql_commands):
    safe_sql = sql.replace('"', '\\"').replace('\n', ' ')
    cmd = f'mysql -u ventas_newhype_prod -p"Tarapoto2026" ventas_newhype_prod -e "{safe_sql}"'
    stdin, stdout, stderr = ssh.exec_command(cmd)
    out = stdout.read().decode().strip()
    err = stderr.read().decode().strip()
    if err and 'warning' not in err.lower() and 'Duplicate column' not in err:
        print(f"   SQL {i+1} error: {err}")
    else:
        print(f"   SQL {i+1}: OK")

# 1. Kill existing process
print("\n1. Killing existing backend process...")
